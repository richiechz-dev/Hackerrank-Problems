...
def arr_len():
    """
    Solución emergente, mas verbosa
    """

    arr_len = [1, 2, 3, 4, 5]

    longitud_arr = len(arr_len)
    suma_min = 0
    suma_max = 0
    total = 0

    for i in range(longitud_arr):
        if i < 4 :
            total = total + arr_len[i]
...
def solution_hackerrank():
    ...
    for i in range(4):
        sumMin = sumMin + arr_ordenado[i]

    for j in range(1,5):
        sumMax = sumMax + arr_ordenado[j]
